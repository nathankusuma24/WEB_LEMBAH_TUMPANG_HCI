...
function smtp_mail($to, $subject, $message, $from_name, $from, $debug=false) {
  $mail = new PHPMailer;
  ...
  $mail->SMTPAuth 	= true;                        				
  $mail->Host 		  = 'lembahtumpang.xyz; 103.145.226.234';  // Masukkan Server SMTP
  $mail->Port 		  = 587;                                      // Masukkan Port SMTP
  $mail->SMTPSecure = 'tls';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
  $mail->Username 	= 'noreply@lembahtumpang.xyz';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
  $mail->Password 	= '13f1964gslianG';        						          // Masukkan Password dari Email tsb
  $default_email_from       = 'noreply@lembahtumpang.xyz';        // Masukkan default from pada email
